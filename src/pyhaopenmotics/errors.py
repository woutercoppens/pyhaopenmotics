...
def client_error_handler() -> Generator[None]:
    try:
        ...
        raise NetworkTimeoutException(
            "Request timed out while accessing the API. Retry the request with longer timeout. Check the network if the issue persists.",
            e.request,
            None,
        ) from e
    except NetworkError as e:
        raise NetworkException(
            "Unknown network error while accessing the API. Retry the request or check the network if the issue persists.",
            e.request,
            None,
        ) from e
    except HTTPStatusError as e:
        if (
            e.response.status_code == codes.UNAUTHORIZED
            or e.response.status_code == codes.FORBIDDEN
        ):
            raise RequestUnauthorizedException(
                "Access token is invalid or expired. Refresh the access token and try again.",
                e.request,
                e.response,
            ) from e
        if e.response.status_code == codes.TOO_MANY_REQUESTS:
            raise RequestBackoffException(
                "Client is sending too many requests. Reduce the frequency at which API is accessed and try again.",
                e.request,
                e.response,
            )
        if codes.is_client_error(e.response.status_code):
            raise RequestClientException(
                "Server processed the request and returned a 4xx response. Don't retry the request without changing the arguments as the new request will fail again with the same error.",
                e.request,
                e.response,
            ) from e
        if codes.is_server_error(e.response.status_code):
            raise RequestServerException(
                "Server couldn't process the request successfully and returned a 5xx response. Try again with reasonable backoff strategy.",
                e.request,
                e.response,
            ) from e
        else:
